...
static void locSrvCrtpCB(CRTPPacket* pk)
{
  ESP_LOGI("LOCSRV", "Received localization CRTP packet on channel %d", pk->channel);
...
static void extPositionHandler(CRTPPacket* pk) {
  ...
  ESP_LOGI("EXTPOSE", "[FLOAT] x=%.3f y=%.3f z=%.3f", data->x, data->y, data->z);
  ...
  // DEBUG: Confirm CRTP packets are received and decoded
