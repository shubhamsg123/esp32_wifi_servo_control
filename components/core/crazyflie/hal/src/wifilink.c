...
#include <stdbool.h>
#include <string.h>
#include "debug_cf.h"   // already included above
...
static void wifilinkTask(void *param)
{
    while (1) {
        ...
        DEBUG_PRINT("WiFi packet len=%u", (unsigned)wifiIn.size);
...
#endif
        {
            ...
            /* command step - receive  04 copy CRTP part from packet, strip checksum */
        size_t crtpLen = wifiIn.size;   // header + payload
        if (crtpLen == 0) {
            // ESP_LOGW("WIFILINK", "Drop empty packet");
            DEBUG_PRINT("Drop empty packet");
            continue;
        }
        if (crtpLen > sizeof(p.raw)) {
            ESP_LOGW("WIFILINK", "Drop packet %u > %u",
                    (unsigned)crtpLen, (unsigned)sizeof(p.raw));
            DEBUG_PRINT("Drop too large packet");        
            continue;
        }

        memcpy(p.raw, wifiIn.data, crtpLen);
        p.size = crtpLen - 1;   // payload only, as CRTP expects

        }

        /* command step - receive 05 send to crtpPacketDelivery queue */
        if (crtpPacketDelivery == NULL) {
            ESP_LOGE("WIFILINK", "crtpPacketDelivery not initialized!");
        } else {
