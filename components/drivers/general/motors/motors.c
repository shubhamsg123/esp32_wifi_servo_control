...
bool pwm_timmer_init()
{
    ...
    ledc_timer_config_t ledc_timer = {
        .duty_resolution = LEDC_TIMER_14_BIT, // 14-bit resolution (max at 50Hz)
        .freq_hz = SERVO_FREQ_HZ,             // 50Hz for MG995 servos
...
/******** MG995 Servo Functions ********/

// Write pulse width in microseconds (1000-2000)
// 1000µs = Full CCW, 1500µs = Stop, 2000µs = Full CW
void servoWriteMicroseconds(uint32_t id, uint16_t us) {
    ...
    // Convert µs to duty at 50Hz (20000µs period), 14-bit resolution (16383 max)
    // duty = (us / 20000) * 16383
    uint32_t duty = (uint32_t)us * 16383 / 20000;
    
    ledc_set_duty(motors_channel[id].speed_mode, 
                  motors_channel[id].channel, duty);
    ledc_update_duty(motors_channel[id].speed_mode, 
                     motors_channel[id].channel);
    motor_ratios[id] = us;
}

// Stop servo (set to neutral 1500µs)
void servoStop(uint32_t id) {
    servoWriteMicroseconds(id, SERVO_MID_US);
}
